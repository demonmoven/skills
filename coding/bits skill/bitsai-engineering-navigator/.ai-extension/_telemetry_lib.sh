#!/bin/sh
# _telemetry_lib.sh — Shared helpers for span_invoke_start.sh / span_invoke_end.sh
...
# ── Debug helper ──────────────────────────────────────────────────────────────
_dbg() {
  [ "${AI_EXTENSION_TELEMETRY_DEBUG:-0}" = "1" ] && printf '[ai-ext-tel] %s\n' "$*" >&2
  return 0
}
...
_send_event() {
  _dbg "payload: $1"
  if [ "${AI_EXTENSION_TELEMETRY_DEBUG:-0}" = "1" ]; then
    curl -s \
