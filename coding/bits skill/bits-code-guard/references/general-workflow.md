...
> 本文档中 `/tmp/<repo>_<session>/` 是简写，实际路径为 SKILL.md Step 2 中创建的 `/tmp/<repo>_<session>/`。
...
读取 `/tmp/<repo>_<session>/review_files.md`，获取：
- 待评审文件列表及路径
- 用户指定的关注范围说明（如有）
...
创建 `/tmp/<repo>_<session>/review_groups.md`，记录分组结果、分组依据和审查重点：
...
- 输出结果保存到 `/tmp/<repo>_<session>/group/group_<num>.jsonl`，每行一个缺陷 JSON 对象
- diff 方向语义：`-` 行是旧代码（已删除），`+` 行是新代码（本次引入），仅对新代码和存续上下文中的问题报告缺陷

### Fallback：无法委派 task agent 时

如果当前运行环境不支持 subagent/task 委派（例如受限的执行器、或 task 工具不可用），当前 agent 按分组顺序在本地串行执行每一组，每组独立完成 Step 3.1–3.4 的完整流程，产物路径和格式保持一致（仍写入 `/tmp/<repo>_<session>/group/group_<num>.jsonl`），不得为了省事合并分组或跳过步骤。

---

## Step 3：执行评审

无论直接评审还是委派的 task agent，均按以下流程执行：
...
收集所有缺陷：
- 直接评审：收集当前评审产出的缺陷列表
- 分组评审：读取所有 `/tmp/<repo>_<session>/group/group_<num>.jsonl`，合并缺陷

将所有缺陷写入 `/tmp/<repo>_<session>/comments.jsonl`（每行一个 JSON 对象）。
每条对象都必须包含非空 `title` 字段，作为缺陷的一句话描述。

### 跨组校验（仅分组评审）

分组评审汇总后，委派一个 task agent 执行跨组校验，检查分组评审的盲区：
- 读取 `/tmp/<repo>_<session>/review_groups.md` 获取分组信息和各组文件列表
- 读取所有 `/tmp/<repo>_<session>/group/group_<num>.jsonl` 获取各组发现的缺陷
- 如果某分组的变更涉及**导出函数/方法的签名变更**（参数增删、类型修改、返回值变化），检查其他分组中该函数的调用方是否已同步更新
- 如果某分组修改了**接口定义或共享类型**，检查实现方和使用方是否一致

发现跨组问题时，补充到 `comments.jsonl` 中。

---

## Step 5：排序、去重、过滤

对 `comments.jsonl` 中的缺陷执行以下处理：
...
**例外**：如果用户指定了具体的文件或函数（检查 `/tmp/<repo>_<session>/review_files.md` 头部是否有"用户指定评审范围"标注），跳过此过滤。用户指定范围意味着希望对这些代码进行完整评审，缺陷不限于 diff 变更行。
...
将结果保存到 `/tmp/<repo>_<session>/final_comments.json`（JSON 数组格式）。

---

## Step 6：生成总结报告

基于 `/tmp/<repo>_<session>/final_comments.json` 生成用户可读的总结报告。
...
- `-o`：输出路径，保存到 `/tmp/<repo>_<session>/report.html`

脚本在生成 HTML 的同时，会在相同目录下输出同名的 `.md` 文件（如 `report.md`）。工作流无需读取 `.md` 内容，脚本自动产出即可。

生成完成后，在 Markdown 报告末尾附上一行链接提示，使用链接语法让用户只看到文件名：
...
HTML 生成完成后，询问用户是否执行 `open /tmp/<repo>_<session>/report.html` 在浏览器中打开报告，由用户确认后再执行。
