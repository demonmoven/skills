...
## 与 openspec 各 action（init/propose/apply/archive）的拼接示例

> openspec 所有 action 都传 `IS_NEW_FEATURE = false`，即不参与分支管理。本流程对 openspec 退化为「git 仓库扫描 + 主仓库选择」，步骤 3（list 单次确认）与步骤 5（创建工作分支）整步跳过。

```text
# 在 openspec 各 action 的 Step 1 调用：

读取并执行 <skill_dir>/prompts/resolve_workspace.md，传入：
  SKILL_KIND     = "openspec"
  SPECS_DIR_NAME = "openspec"
  FEATURE_NAME   = {CHANGE_NAME}
  IS_NEW_FEATURE = false              # openspec 永远传 false

获得：
  PRIMARY_REPO
  PRIMARY_BRANCH                       # = selections[PRIMARY_REPO].base_branch（即主仓库当前分支）
  WORKSPACE_REPOS

后续：
  CWD = PRIMARY_REPO   # 取代原来的 $(pwd)
  CHANGES_DIR = {PRIMARY_REPO}/docs/xdev/openspec/changes/{CHANGE_NAME}
  mkdir -p {CHANGES_DIR}
  写入 proposal.md 等
```
