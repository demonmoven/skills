...
**Question:** "Plan ready at `docs/xdev/ce/plans/YYYY-MM-DD-NNN-<type>-<name>-plan.md`. What would you like to do next?"
...
Based on selection:
- **Open plan in editor** -> Open `docs/xdev/ce/plans/<plan_filename>.md` using the current platform's file-open or editor mechanism (e.g., `open` on macOS, `xdg-open` on Linux, or the IDE's file-open API)
...
- **Share to Proof** -> Upload the plan:
  ```bash
  CONTENT=$(cat docs/xdev/ce/plans/<plan_filename>.md)
