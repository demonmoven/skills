> 移植自 [EveryInc/compound-engineering-plugin/ce-review](https://github.com/EveryInc/compound-engineering-plugin) v2.66.1（MIT）
...
| `plan:<path>` | `plan:docs/xdev/ce/plans/2026-03-25-001-feat-foo-plan.md` | Load this plan for requirements verification |
...
- **Write a run artifact** under `docs/xdev/ce/reviews/<run-id>/` summarizing findings, applied fixes, residual actionable work, and advisory outputs.
...
- **Never edit files or externalize work.** Do not write `docs/xdev/ce/reviews/<run-id>/`, do not create todo files, and do not commit, push, or create a PR.
...
- **Write a run artifact** under `docs/xdev/ce/reviews/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
| `compound-engineering:research:learnings-researcher` | Search docs/xdev/ce/solutions/ for past issues related to this PR |
...
- `docs/xdev/ce/brainstorms/*` -- requirements documents created by ce:brainstorm
- `docs/xdev/ce/plans/*.md` -- plan files created by ce:plan (living documents with progress checkboxes)
- `docs/xdev/ce/solutions/*.md` -- solution documents created during the pipeline
...
Then detect the review base branch and compute the merge-base. Run the `<skill_dir>/references/review/resolve-base.sh` script, which handles fork-safe remote resolution with multi-fallback detection (PR metadata -> `origin/HEAD` -> `gh repo view` -> common branch names):

```
RESOLVE_OUT=$(bash <skill_dir>/references/review/resolve-base.sh) || { echo "ERROR: resolve-base.sh failed"; exit 1; }
...
Detect the review base branch and compute the merge-base using the same `<skill_dir>/references/review/resolve-base.sh` script as branch mode:
...
2. **PR body.** If PR metadata was fetched in Stage 1, scan the body for paths matching `docs/xdev/ce/plans/*.md`. If exactly one match is found and the file exists, use it as `plan_source: explicit`. If multiple plan paths appear, treat as ambiguous — demote to `plan_source: inferred` for the most recent match that exists on disk, or skip if none exist or none clearly relate to the PR title/intent. Always verify the selected file exists before using it — stale or copied plan links in PR descriptions are common.
3. **Auto-discover.** Extract 2-3 keywords from the branch name (e.g., `feat/onboarding-skill` -> `onboarding`, `skill`). Glob `docs/xdev/ce/plans/*` and filter filenames containing those keywords. If exactly one match, use it. If multiple matches or the match looks ambiguous (e.g., generic keywords like `review`, `fix`, `update` that could hit many plans), **skip auto-discovery** — a wrong plan is worse than no plan. If zero matches, skip.
...
mkdir -p "docs/xdev/ce/reviews/$RUN_ID"
```

Pass `{run_id}` to every persona sub-agent so they can write their full analysis to `docs/xdev/ce/reviews/{run_id}/{reviewer_name}.json`.
...
Each persona sub-agent writes full JSON (all schema fields) to `docs/xdev/ce/reviews/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
7. **Learnings & Past Solutions.** Surface learnings-researcher results: if past solutions are relevant, flag them as "Known Pattern" with links to docs/xdev/ce/solutions/ files.
...
Artifact: docs/xdev/ce/reviews/<run-id>/
...
**Detail enrichment (headless only):** The headless envelope includes `Why:`, `Evidence:`, and `Suggested fix:` lines. After merge (Stage 5), read the per-agent artifact files from `docs/xdev/ce/reviews/{run_id}/` for only the findings that survived dedup and confidence gating.
...
5. **Protected artifacts are respected.** Discard any findings that recommend deleting or gitignoring files in `docs/xdev/ce/brainstorms/`, `docs/xdev/ce/plans/`, or `docs/xdev/ce/solutions/`.
...
- In interactive, autofix, and headless modes, write a per-run artifact under `docs/xdev/ce/reviews/<run-id>/` containing:
...
@./<skill_dir>/references/review/persona-catalog.md

### Subagent Template

@./<skill_dir>/references/review/subagent-template.md

### Diff Scope Rules

@./<skill_dir>/references/review/diff-scope.md

### Findings Schema

@./<skill_dir>/references/review/findings-schema.json

### Review Output Template

@./<skill_dir>/references/review/review-output-template.md
