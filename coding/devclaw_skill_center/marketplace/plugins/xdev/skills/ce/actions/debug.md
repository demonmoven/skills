> 移植自 [EveryInc/compound-engineering-plugin/ce-debug](https://github.com/EveryInc/compound-engineering-plugin) v2.66.1（MIT）
...
- **Does not reproduce after 2-3 attempts:** Read `<skill_dir>/references/debug/investigation-techniques.md` for intermittent-bug techniques.
...
- If the bug looks like a regression ("it worked before"), use `git bisect` (see `<skill_dir>/references/debug/investigation-techniques.md`)
...
Read `<skill_dir>/references/debug/anti-patterns.md` before forming hypotheses.
