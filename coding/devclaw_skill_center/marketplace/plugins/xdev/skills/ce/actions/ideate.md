> 移植自 [EveryInc/compound-engineering-plugin/ce-ideate](https://github.com/EveryInc/compound-engineering-plugin) v2.66.1（MIT）
...
This workflow produces a ranked ideation artifact in `docs/xdev/ce/ideation/`. It does **not** produce requirements, plans, or code.
...
Look in `docs/xdev/ce/ideation/` for ideation documents created within the last 30 days.
...
- **Past learnings** — relevant institutional knowledge from docs/xdev/ce/solutions/
...
After merging and synthesis, read `<skill_dir>/references/ideate/post-ideation-workflow.md` for the adversarial filtering rubric, presentation format, artifact template, handoff options, and quality bar. Do not load this file before Phase 2 agent dispatch completes.
