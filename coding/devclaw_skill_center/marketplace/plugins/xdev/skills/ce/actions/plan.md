> 移植自 [EveryInc/compound-engineering-plugin/ce-plan](https://github.com/EveryInc/compound-engineering-plugin) v2.66.1（MIT）
...
If the user references an existing plan file or there is an obvious recent matching plan in `docs/xdev/ce/plans/`:
- Read it
- Confirm whether to update it in place or create a new plan
- If updating, preserve completed checkboxes and revise only the still-relevant sections

**Deepen intent:** The word "deepen" (or "deepening") in reference to a plan is the primary trigger for the deepening fast path. When the user says "deepen the plan", "deepen my plan", "run a deepening pass", or similar, the target document is a **plan** in `docs/xdev/ce/plans/`, not a requirements document. Use any path, keyword, or context the user provides to identify the right plan. If a path is provided, verify it is actually a plan document. If the match is not obvious, confirm with the user before proceeding.
...
- If the plan lacks YAML frontmatter (non-software plans use a simple `# Title` heading with `Created:` date instead of frontmatter), route to `<skill_dir>/references/plan/universal-planning.md` for editing or deepening instead of Phase 5.3. Non-software plans do not use the software confidence check.
...
If the task is about a non-software domain and describes a multi-step goal worth planning, read `<skill_dir>/references/plan/universal-planning.md` and follow that workflow instead. Skip all subsequent phases.
...
Before asking planning questions, search `docs/xdev/ce/brainstorms/` for files matching `*-requirements.md`.
...
- Institutional learnings from `docs/xdev/ce/solutions/`
...
- Build the filename following the repository convention: `docs/xdev/ce/plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md`
  - Create `docs/xdev/ce/plans/` if it does not exist
...
status: active
date: YYYY-MM-DD
origin: docs/xdev/ce/brainstorms/YYYY-MM-DD-<topic>-requirements.md  # include when planning from a requirements doc
...
- [Relevant `docs/xdev/ce/solutions/` insight]
...
- **Origin document:** [docs/xdev/ce/brainstorms/YYYY-MM-DD-<topic>-requirements.md](path)
...
```text
docs/xdev/ce/plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md
```

Confirm:

```text
Plan written to docs/xdev/ce/plans/[filename]
```
