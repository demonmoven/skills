> 移植自 [EveryInc/compound-engineering-plugin/ce-compound](https://github.com/EveryInc/compound-engineering-plugin) v2.66.1（MIT）
> 上游 frontmatter / config 系统 / model 指定 / telemetry 已删除，核心工作流逻辑保留。产物目录对齐 xdev 规范。
...
Captures problem solutions while context is fresh, creating structured documentation in `docs/xdev/ce/solutions/` with YAML frontmatter for searchability and future reference. Uses parallel subagents for maximum efficiency.
...
- `<skill_dir>/references/compound/schema.yaml` — canonical frontmatter fields and enum values (read when validating YAML)
- `<skill_dir>/references/compound/yaml-schema.md` — category mapping from problem_type to directory (read when classifying)
- `<skill_dir>/references/compound/resolution-template.md` — section structure for new docs (read when assembling)
...
#### 1. **Context Analyzer**
   ...
   - Reads `<skill_dir>/references/compound/schema.yaml` for enum validation and **track classification**
   ...
   - Reads `<skill_dir>/references/compound/yaml-schema.md` for category mapping into `docs/xdev/ce/solutions/`
...
#### 2. **Solution Extractor**
   - Reads `<skill_dir>/references/compound/schema.yaml` for track classification (bug vs knowledge)
...
#### 3. **Related Docs Finder**
   - Searches `docs/xdev/ce/solutions/` for related documentation
   ...
   **Search strategy (grep-first filtering for efficiency):**
   ...
   2. If the problem category is clear, narrow search to the matching `docs/xdev/ce/solutions/<category>/` directory
...
4. Assemble complete markdown file from the collected pieces, reading `<skill_dir>/references/compound/resolution-template.md` for the section structure of new docs
5. Validate YAML frontmatter against `<skill_dir>/references/compound/schema.yaml`
6. Create directory if needed: `mkdir -p docs/xdev/ce/solutions/[category]/`
7. Write the file: either the updated existing doc or the new `docs/xdev/ce/solutions/[category]/[filename].md`

When creating a new doc, preserve the section order from `<skill_dir>/references/compound/resolution-template.md` unless the user explicitly asks for a different structure.
...
- **Pattern filename or pattern topic** when the stale guidance lives in `docs/xdev/ce/solutions/patterns/`
...
After the learning is written and the refresh decision is made, check whether the project's instruction files would lead an agent to discover and search `docs/xdev/ce/solutions/` before starting work in a documented area. This runs every time — the knowledge store only compounds value when agents can find it.
...
2. Assess whether an agent reading the instruction files would learn three things:
   ...
   This is a semantic assessment, not a string match. The information could be a line in an architecture section, a bullet in a gotchas section, spread across multiple places, or expressed without ever using the exact path `docs/xdev/ce/solutions/`. Use judgment — if an agent would reasonably discover and use the knowledge store after reading the file, the check passes.
...
4. If not:
   ...
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density. The addition should describe the knowledge store itself, not the plugin — an agent without the plugin should still find value in it.
      ...
      ```
      docs/xdev/ce/solutions/  # documented solutions to past problems (bugs, best practices, workflow patterns), organized by category with YAML frontmatter (module, tags, problem_type)
      ```
      ...
      `docs/xdev/ce/solutions/` — documented solutions to past problems (bugs, best practices, workflow patterns), organized by category with YAML frontmatter (`module`, `tags`, `problem_type`). Relevant when implementing or debugging in documented areas.
      ```
...
2. **Classify**: Read `<skill_dir>/references/compound/schema.yaml` and `<skill_dir>/references/compound/yaml-schema.md`, then determine track (bug vs knowledge), category, and filename
3. **Write minimal doc**: Create `docs/xdev/ce/solutions/[category]/[filename].md` using the appropriate track template from `<skill_dir>/references/compound/resolution-template.md`, with:
...
File created:
- docs/xdev/ce/solutions/[category]/[filename].md

[If discoverability check found instruction files don't surface the knowledge store:]
Tip: Your AGENTS.md/CLAUDE.md doesn't surface docs/xdev/ce/solutions/ to agents —
...
- File: `docs/xdev/ce/solutions/[category]/[filename].md`
...
| Multiple files created during workflow | One solution doc written or updated: `docs/xdev/ce/solutions/[category]/[filename].md` (plus an optional small edit to a project instruction file for discoverability) |
...
File created:
- docs/xdev/ce/solutions/performance-issues/n-plus-one-brief-generation.md
...
Overlap detected: docs/xdev/ce/solutions/performance-issues/n-plus-one-queries.md
...
File updated:
- docs/xdev/ce/solutions/performance-issues/n-plus-one-queries.md (added last_updated: 2026-03-24)
```
...
2. Document the solution → docs/xdev/ce/solutions/performance-issues/n-plus-one-briefs.md (5 min)
...
Writes the final learning directly into `docs/xdev/ce/solutions/`.
...
- `/research [topic]` - Deep investigation (searches docs/xdev/ce/solutions/ for patterns)
