> 移植自 [EveryInc/compound-engineering-plugin/ce-optimize](https://github.com/EveryInc/compound-engineering-plugin) v2.66.1（MIT）
...
`<skill_dir>/references/optimize/optimize-spec-schema.yaml`

## Experiment Log Schema

Reference the experiment log schema for state management:

`<skill_dir>/references/optimize/experiment-log-schema.yaml`

## Quick Start

For a first run, optimize for signal and safety, not maximum throughput:

- Start from `<skill_dir>/references/optimize/example-hard-spec.yaml` when the metric is objective and cheap to measure
- Use `<skill_dir>/references/optimize/example-judge-spec.yaml` only when actual quality requires semantic judgment
...
`<skill_dir>/references/optimize/usage-guide.md`
...
2. Validate against `<skill_dir>/references/optimize/optimize-spec-schema.yaml`:
...
2. Include all required top-level sections from `<skill_dir>/references/optimize/experiment-log-schema.yaml`: `spec`, `run_id`, `started_at`, `baseline`, `experiments`, and `best`
...
3. Fill the experiment prompt template (`<skill_dir>/references/optimize/experiment-prompt-template.md`) with:
...
5. **If gates pass AND primary type is `judge`**:
   ...
   - Fill the judge prompt template (`<skill_dir>/references/optimize/judge-prompt-template.md`) for each batch
