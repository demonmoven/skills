> 移植自 [EveryInc/compound-engineering-plugin/ce-brainstorm](https://github.com/EveryInc/compound-engineering-plugin) v2.66.1（MIT）
...
If the user references an existing brainstorm topic or document, or there is an obvious recent matching `*-requirements.md` file in `docs/xdev/ce/brainstorms/`:
- Read the document
...
**If non-software brainstorming is detected:** Read `<skill_dir>/references/brainstorm/universal-brainstorming.md` and use those facilitation principles to brainstorm with the user naturally. Do not follow the software brainstorming phases below.
...
Write or update a requirements document only when the conversation produced durable decisions worth preserving. Read `<skill_dir>/references/brainstorm/requirements-capture.md` for the document template, formatting rules, visual aid guidance, and completeness checks.
...
Present next-step options and execute the user's selection. Read `<skill_dir>/references/brainstorm/handoff.md` for the option logic, dispatch instructions, and closing summary format.
