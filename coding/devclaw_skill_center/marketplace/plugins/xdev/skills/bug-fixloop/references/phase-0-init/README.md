...
## 对应原 Stage
原 byted-bug-fixloop 的 phase-0-precheck（前置检查）+ bug-fixloop 新增的"交互式参数收集"。

## 干什么
bug-fixloop 的初始化阶段，分两步执行：
...
## PROFILE 适配（bug-fixloop 新增）

bug-fixloop 在 wizard 最前面新增 Step 0 让用户选择 PROFILE：

| PROFILE | 描述 | 必填参数 |
|---------|------|---------|
| `bytedance-tce` | 字节内场（默认推荐） | PSM, BRANCH, BUSINESS_REPO, **COMMIT_RANGE**, IDL_REPO, IDL_BRANCH |
| `none` | 自部署（最简化） | BRANCH, BUSINESS_REPO, **COMMIT_RANGE**, BASE_URL |
...
- 核心必填参数：BUSINESS_REPO / BRANCH / COMMIT_RANGE
- PROFILE=bytedance-tce 时额外：PSM / IDL_REPO / IDL_BRANCH
- PROFILE=none 时额外：BASE_URL
