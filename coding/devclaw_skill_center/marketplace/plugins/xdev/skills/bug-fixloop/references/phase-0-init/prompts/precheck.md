...
# 验证目录可达（bug-fixloop 不需要 SPEC_DIR）
ls $BUSINESS_REPO_PATH
if [ "$PROFILE" = "bytedance-tce" ]; then
  ls $IDL_REPO_PATH
fi

# 验证 COMMIT_RANGE 合法（phase-1 step1-diff-parse 会再校验一次,这里是早期快速失败）
if [ -n "$COMMIT_RANGE" ]; then
  cd $BUSINESS_REPO_PATH
  git rev-parse --verify "${COMMIT_RANGE%..*}^{commit}" >/dev/null 2>&1 || {
    echo "ERROR: COMMIT_RANGE 起点无效: ${COMMIT_RANGE%..*}"; exit 1;
  }
fi
```
