---
name: gan-fixloop
description: 全链路测试修复循环（通用版，PROFILE 化适配）：通过 PROFILE 选择运行环境（bytedance-tce / none），支持字节内场 TCE 部署或自部署模式；保留 zero-arg 启动 + 交互式参数收集；用户可直接 /gan-fixloop 不带任何参数启动。
argument-hint: "（可选，zero-arg 即可启动；显式参数会跳过对应 wizard step）"
---

# gan-fixloop: 全链路测试修复循环
...
"你的任务是执行 gan-fixloop Phase 1 Step 1：用户动线提取。

请先读取指南文件：/path/to/skills/gan-fixloop/references/phase-1-craft-test-case/prompts/step1-user-journeys.md
然后按照其中的流程执行。
...
gan-fixloop 支持 **zero-arg 启动**：用户可以直接 `/gan-fixloop` 不带任何参数运行，所有必填参数会通过 **Phase 0 init** 阶段的交互式 wizard 自动收集。

### 调用姿势

| 姿势 | 命令 | wizard 行为 |
|------|------|----------|
| **zero-arg** | `/gan-fixloop` | 全部参数通过 wizard 交互收集 |
| **半自动** | `/gan-fixloop PSM=xxx BRANCH=main` | 已显式给出的参数跳过对应 question，缺失的进入 wizard |
| **完全显式（兼容老用法）** | `/gan-fixloop PSM=... BRANCH=... BUSINESS_REPO=... TEST_REPO=... SPEC_DIR=... IDL_REPO=... IDL_BRANCH=...` | wizard 跳过所有 question，仅最终汇总确认 |
...
详见 `references/phase-0-init/prompts/interactive-collect.md`。该文件定义了 5 步 wizard（gan-fixloop 在 new-byted-gan-fixloop 基础上新增 Step 0）：

1. **Step 0/5**：PROFILE 选择（bytedance-tce / none）← gan-fixloop 新增
2. **Step 1/5**：仓库与分支（BUSINESS_REPO / BRANCH / TEST_REPO）
3. **Step 2/5**：输入源
   - PROFILE=bytedance-tce: SPEC_DIR / IDL_REPO / IDL_BRANCH
   - PROFILE=none: SPEC_DIR / BASE_URL
...
- **zero-arg 启动**：用户直接 `/gan-fixloop` 不带参数，wizard 自动从 cwd / git / 文件系统扫描候选并让用户选择
...
> **gan-fixloop 改造说明**：原独立的 Stage 1.5 内容已经合并到 Stage 1 的 `adapters/bytedance-tce.md` 文件中（"AGW IDL 同步（合并自原 stage-1.5）"段）。
>
