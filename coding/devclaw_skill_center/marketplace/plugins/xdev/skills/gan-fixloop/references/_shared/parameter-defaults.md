...
1. **上下文推断优先**：gan-fixloop 经常在 speckit 等上游 skill 之后运行，对话上下文中可能已包含 BUSINESS_REPO（CWD）、BRANCH（当前分支）、SPEC_DIR（specs 目录）、IDL_REPO、IDL_BRANCH 等信息。对缺失的必填参数，先扫描对话上下文尝试推断。
...
```bash
# 完整参数（无需交互）
/gan-fixloop PSM=stone.cozeloop.prompt BRANCH=main BUSINESS_REPO=/path/backend TEST_REPO=/path/test SPEC_DIR=/path/specs IDL_REPO=/path/idl IDL_BRANCH=main MAX_ITERATIONS=5

# 部分参数（交互补充缺失的必填参数）
/gan-fixloop PSM=stone.cozeloop.prompt

# 仅跑一次测试
/gan-fixloop PSM=xxx BRANCH=main BUSINESS_REPO=/path/backend TEST_REPO=/path/test SPEC_DIR=/path/specs IDL_REPO=/path/idl IDL_BRANCH=main SINGLE_TEST_RUN=true GENERATE_TESTS=false

# 跳过测试生成，直接进入修复循环
/gan-fixloop PSM=xxx BRANCH=main BUSINESS_REPO=/path/backend TEST_REPO=/path/test SPEC_DIR=/path/specs IDL_REPO=/path/idl IDL_BRANCH=main GENERATE_TESTS=false
```
