...
```
[Step 0/5] PROFILE 选择 (bytedance-tce / none) ← gan-fixloop 新增
   ↓
[预热] cwd 扫描（收集所有候选）
   ↓
...
> **关键改动（gan-fixloop）**：相比 new-byted-gan-fixloop，本 wizard 在最前面新增 Step 0 PROFILE 选择，后续 step 根据 PROFILE 动态决定哪些 question 发出。

## Step 0/5 — PROFILE 选择（gan-fixloop 新增）
...
如果用户选 `bytedance-tce`：
- 行为与 new-byted-gan-fixloop 完全相同（保留所有原有 question）
...
4. **已显式给出的参数自动跳过对应 question**。比如用户调用 `/gan-fixloop PSM=stone.cozeloop.prompt`，PSM 的 question 不发出。
...
> - **PROFILE=bytedance-tce**：发出 SPEC_DIR / IDL_REPO / IDL_BRANCH 三个 question
> - **PROFILE=none**：发出 SPEC_DIR / BASE_URL 两个 question（不需要 IDL）
...
- **header**: "SPEC 目录"
- **question**: "SPEC 需求文档目录 SPEC_DIR"
- **options**:
  ```
  - "{spec_candidate_1}  (cwd 内 specs/, 推荐)"
  - "{spec_candidate_2}  (cwd 内 openspec/)"
  - "{spec_candidate_3}  (父目录里的 *-specs/)"
  - "手动输入"
  ```
...
### Question 5b: BASE_URL  ⚠️ 仅 PROFILE=none（gan-fixloop 新增）
...
```bash
# 例：用户调用 /gan-fixloop PSM=stone.cozeloop.prompt BRANCH=main
EXPLICIT_PARAMS=()
