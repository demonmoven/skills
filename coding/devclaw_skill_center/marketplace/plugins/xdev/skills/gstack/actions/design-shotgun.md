...
> 移植自 [gstack/design-shotgun](https://github.com/garrytan/gstack/blob/main/design-shotgun/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/design-shotgun.md`。
>
> ⚠️ **前置依赖**：本 action 需要 gstack 上游的 `browse` + `design` 二进制（`$B` / `$D`）。请先安装：
...
> 二进制路径：`~/.claude/skills/gstack/bin/browse`、`~/.claude/skills/gstack/design/dist/design`

## 参数

- `ARG`（可选）：要 brainstorm 的 screen 或 UI feature 描述（如「landing page hero」「checkout flow」「dashboard card layout」）、想探索的方向数量（默认 3，最多 8）、以及已有 screenshot/URL 作为 evolve 基准（如「I don't like this」配 URL）。可为空——若为空则进入 Phase 1 Context Gathering 从上下文推断。

## 入参校验

读取并执行 `<skill_dir>/prompts/resolve_action_target.md`，传入：
- `ACTION_NAME = "design-shotgun"`
- `ARG = {用户传入的参数}`
...
> design-shotgun 需要 gstack 的 browse 浏览器二进制（用于打开 comparison board）和 design 二进制（用于生成 mockup 变体）。当前未检测到。请选择：
>
...
> B) 切换为纯文本模式（仅产出 text 概念描述 + 用 `open` 打开 HTML comparison board，不生成 PNG mockup）
> C) 中止本次 action
...
- **Boil the Lake**：design-shotgun 要一次性并行生成多个（3-8 个）完整变体，不要只生成一个"够用的"就收手。Parallel generation 让 N 个变体和 1 个变体的时间成本几乎一样——完整的多样性探索是 AI near-zero cost 时代的默认。
- **User Sovereignty**：最终选哪个变体、是否继续 iterate、是否 remix elements，**完全由用户拍板**。你只负责生成、组织 comparison board、忠实呈现反馈。Comparison board 就是 chooser，不要用 AskUserQuestion 替用户做选择。绝不"代替用户决定"。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/design-shotgun.md`：

- 若文件已存在则**在文件末尾追加**新一轮的输出（用 `## 第 N 次执行 ({YYYY-MM-DD HH:MM:SS+TZ})` 作为分隔标题）
- 若文件不存在则创建并写入完整结构
- 该 action 产生的 variant PNG、comparison board HTML、approved.json、feedback.json 等附件统一放到 `{SPECS_DIR}/artifacts/`，并在产物文件中以相对路径引用
- 产物文件需包含：brief、concepts（按字母列出的方向）、每个 variant 的一句描述 + 相对路径、用户最终选的 variant + ratings + comments + direction、next steps

执行完成后，输出一行简短摘要：

```
✓ design-shotgun 完成。报告已写入 {SPECS_DIR}/design-shotgun.md
```
