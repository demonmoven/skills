...
> 移植自 [gstack/cso](https://github.com/garrytan/gstack/blob/main/cso/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/cso.md`。

## 参数

- `ARG`(可选)：扫描模式或范围限定。可为空——若为空则按 daily 模式跑全部 phase 0-14。支持的形态包括 `--comprehensive`、`--infra`、`--code`、`--skills`、`--diff`、`--supply-chain`、`--owasp`、`--scope <domain>` 等(详见下文 Arguments / Mode Resolution)。
...
- **Boil the Lake**：CSO 审计必须跑完所有 phase(secrets archaeology + supply chain + CI/CD + infra + webhooks + LLM + skills + OWASP Top 10 + STRIDE)，不能因为某个领域看似干净就跳过。一次完整扫描比 N 次零散扫描更有价值。
- **User Sovereignty**：所有 CRITICAL/HIGH 级别发现必须通过 AskUserQuestion 询问用户决策(Fix now / Mitigate / Accept risk / Defer)。CSO 是诊断角色，不直接修代码；最终的 remediation 路径由用户拍板。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/cso.md`：
