...
> 移植自 [gstack/retro](https://github.com/garrytan/gstack/blob/main/retro/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/retro.md`。

## 参数

- `ARG`(可选)：时间窗口或模式。支持 `24h` / `7d` / `14d` / `30d` / `compare` / `compare 14d` / `global` / `global 14d` 等。不传时默认 `7d`。
...
- **Boil the Lake**：retro 覆盖整个时间窗口的 14+ 步全量分析——commits、sessions、hotspots、team breakdown、plan completion、test health、streak、trends，都要做完整不留缺口。
- **Search Before Building**：在生成 insights 之前，读取 `.context/retros/` 与 `~/.gstack/retros/` 的历史快照作对比，基于真实历史数据而非当下感觉下结论。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/retro.md`：
...
```
✓ retro 完成。报告已写入 {SPECS_DIR}/retro.md
```
