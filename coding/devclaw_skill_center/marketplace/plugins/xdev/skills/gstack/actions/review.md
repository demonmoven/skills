...
> 移植自 [gstack/review](https://github.com/garrytan/gstack/blob/main/review/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/review.md`。

## 参数

- `ARG`(可选)：base 分支名或其他 review 上下文线索。可为空——若为空则自动检测 base 分支并对当前分支的 diff 做 pre-landing review。
...
> 若本 action 是被 `/gstack run` 派发(pipeline 模式)，调用方已经预先设置了上述变量，本步骤检测到 `SPECS_DIR` 环境变量已存在则跳过解析直接复用。

## 哲学约束

执行本 action 前，读取 `<skill_dir>/resources/ethos.md` 并将其内容作为隐含约束。关键原则(针对本 action)：

- **Boil the Lake**：review 必须跑完 7 个 checklist 维度 + 所有 specialist 分支 + adversarial pass，不能止于 happy path，也不能因为 diff 小就跳过关键类别。
- **Investigation Before Fix**：每个 finding 在给出 fix 之前必须先 cite file:line 证据，cite 具体机制，禁止「大概」「可能」的发现。
- **Atomic Commits**：AUTO-FIX 与 ASK 分离，fix 必须独立可评审；review 本身绝不 commit / push / 创建 PR，那是 `/ship` 的职责。
...
1. **Testing** — read `<skill_dir>/specialists/testing.md`
2. **Maintainability** — read `<skill_dir>/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `<skill_dir>/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `<skill_dir>/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `<skill_dir>/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `<skill_dir>/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `<skill_dir>/design-checklist.md`
...
1. The red-team checklist from `<skill_dir>/specialists/red-team.md`
...
本 action 的关键输出必须写入 `{SPECS_DIR}/review.md`：

- 若文件已存在则**在文件末尾追加**新一轮的输出(用 `## 第 N 次执行 ({YYYY-MM-DD HH:MM:SS+TZ})` 作为分隔标题)
- 若文件不存在则创建并写入完整结构
- 该 action 产生的截图、日志等附件统一放到 `{SPECS_DIR}/artifacts/`，并在产物文件中以相对路径引用

执行完成后，输出一行简短摘要给用户：

```
✓ review 完成。报告已写入 {SPECS_DIR}/review.md
```
