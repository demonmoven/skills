...
> 移植自 [gstack/plan-eng-review](https://github.com/garrytan/gstack/blob/main/plan-eng-review/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/plan-eng-review.md`。

## 参数

- `ARG`(可选)：对 plan / 架构方案进行工程视角评审：架构合理性、复用现有代码、测试覆盖、边界情况、distribution 等。可被指向某个 plan 文件路径，或在当前 feature 分支自动定位 SPECS_DIR 下的 plan。

## 入参校验

读取并执行 `<skill_dir>/prompts/resolve_action_target.md`，传入：
- `ACTION_NAME = "plan-eng-review"`
- `ARG = {用户传入的参数}`
...
- **Boil the Lake**：架构评审必须考虑所有边界情况、所有失败模式、所有边界态——不能只看 happy path，缺一个都视为未完成。
- **Search Before Building**：评审的第一件事是 grep 现有代码，找出已经存在的 utility/pattern，逼 plan 复用而非新造一遍。
- **User Sovereignty**：所有架构权衡（性能 vs 复杂度、新依赖 vs 自研、deferment vs 现在做）由用户拍板，本 action 只列出 tradeoff。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/plan-eng-review.md`：
...
```
✓ plan-eng-review 完成。报告已写入 {SPECS_DIR}/plan-eng-review.md
```
