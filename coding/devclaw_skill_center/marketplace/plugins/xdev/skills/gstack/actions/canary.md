...
> 移植自 [gstack/canary](https://github.com/garrytan/gstack/blob/main/canary/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/canary.md`。

## 参数

- `ARG`(可选)：监控目标 URL 与模式。支持 `<url>`、`<url> --duration 5m`、`<url> --baseline`、`<url> --pages /,/dashboard`、`<url> --quick` 等形式。
...
- **Investigation Before Fix**：canary 捕捉到异常时首选调查而非立即回滚，先定位是部署引起的真回归还是网络/CDN 瞬态噪声，保留第一现场 evidence (截图、console、perf)。
- **User Sovereignty**：CRITICAL/HIGH 级别的异常必须通过 AskUserQuestion 交给用户决定 rollback / continue / investigate / dismiss，绝不自行回滚生产。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/canary.md`：
...
```
✓ canary 完成。报告已写入 {SPECS_DIR}/canary.md
```
