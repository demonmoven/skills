...
> 移植自 [gstack/health](https://github.com/garrytan/gstack/blob/main/health/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/health.md`。
>
> ⚠️ **本仓库特殊说明**：本 skill 的"代码健康度仪表板"依赖 `~/.gstack/projects/{slug}/health-history.jsonl` 状态目录以及 `~/.claude/skills/gstack/bin/gstack-slug` 等上游二进制来写入历史趋势。在本仓库版本中，保留原始路径，即依赖用户独立安装上游 gstack 才能持久化趋势。若没有上游 gstack，仍可运行一次性仪表板并把结果写入 `{SPECS_DIR}/health.md` 作为文本快照。

## 参数

- `ARG`(可选)：自定义健康栈覆盖(tool 子集)或运行目标说明。可为空——默认读取项目 `CLAUDE.md` 的 `## Health Stack` 段或执行自动探测。
...
- **Boil the Lake**：health 必须覆盖代码全维度——typecheck、lint、test、deadcode、shell lint 一个都不能少。缺哪一项就在仪表板上明确标注 `SKIPPED` 并解释原因，不能悄悄忽略导致得分虚高。
- **User Sovereignty**：仪表板只负责诊断，绝不自动修任何问题。即使工具可以 `--fix`，最终是否动手也由用户拍板。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/health.md`：

- 若文件已存在则**在文件末尾追加**新一轮的输出(用 `## 第 N 次执行 ({YYYY-MM-DD HH:MM:SS+TZ})` 作为分隔标题)
- 若文件不存在则创建并写入完整结构
- 仪表板表格、趋势表、回归详情、REGRESSIONS、RECOMMENDATIONS 都要按原样落盘；若 `~/.gstack/projects/{slug}/health-history.jsonl` 可写，也会同步追加一行 JSONL

执行完成后，输出一行简短摘要给用户：

```
✓ health 完成。报告已写入 {SPECS_DIR}/health.md
```
