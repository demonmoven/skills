...
> 移植自 [gstack/codex](https://github.com/garrytan/gstack/blob/main/codex/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/codex.md`。

## 参数

- `ARG`(可选)：子模式及指令。支持：
  - 空 — 自动检测(diff 存在则走 review/challenge；没有 diff 则找 plan 文件走 consult)
  - `review [instructions]` — Review 模式(对 diff 做代码审查 + gate)
  - `challenge [focus]` — 对抗挑战模式(找破绽/边界/漏洞)
  - `<自由问题>` — Consult 模式(问 Codex 任何事，支持 session 续聊)
  - `--xhigh` 任意位置可加 — 最大推理强度(耗时更长)
...
- **Search Before Building**：codex 是"向外求证"的环节 —— 让另一个模型独立调研、独立判断。不要把 Claude 自己的分析喂给 Codex 当上下文(plan review 除外，那是目标物)，也不要用自己的推理替它走过场。Codex 的价值就在于它的视角与 Claude 不同。
- **User Sovereignty**：Codex 的输出是"第二意见"，不是"最终答案"。即使 Claude 和 Codex 在某个 finding 上达成共识，也必须把完整结论呈现给用户，由用户决定是否接受。禁止 "Codex 说了所以就改" 的自动化路径。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/codex.md`：
...
```
✓ codex 完成。报告已写入 {SPECS_DIR}/codex.md
```
