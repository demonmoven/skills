...
> 移植自 [gstack/plan-ceo-review](https://github.com/garrytan/gstack/blob/main/plan-ceo-review/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/plan-ceo-review.md`。

## 参数

- `ARG`(可选)：对 plan / spec / 设计文档进行 CEO/founder-mode 评审。可被指向某个 plan 文件路径，或在当前 feature 分支自动定位 SPECS_DIR 下的 plan。

## 入参校验

读取并执行 `<skill_dir>/prompts/resolve_action_target.md`，传入：
- `ACTION_NAME = "plan-ceo-review"`
- `ARG = {用户传入的参数}`
...
- **User Sovereignty**：CEO 视角是用户视角的代理——所有 scope 扩张/收缩的提议必须由用户拍板，本 action 只负责提出 4 种 mode 的取舍。
- **Boil the Lake**：不要满足于评出 3 星方案，要逼问到 10 星产品的可能性，把所有 scope expansion、premise challenge 都跑完。
- **Search Before Building**：在挑战 plan 前先看现有代码、现有 design doc、conventional 解法，避免在已经回答过的问题上重新挑战。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/plan-ceo-review.md`：
...
```
✓ plan-ceo-review 完成。报告已写入 {SPECS_DIR}/plan-ceo-review.md
```
