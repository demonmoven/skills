...
> 移植自 [gstack/autoplan](https://github.com/garrytan/gstack/blob/main/autoplan/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/autoplan.md`。

## 参数

- `ARG`(可选)：plan 文件路径或额外审查线索。可为空——若为空则自动检测当前分支对应的 plan 文件并串联 CEO / Design / Eng 三段 review。
...
- **Boil the Lake**：CEO / Design(若有 UI 范围) / Eng 三段 review 必须**全部跑完**，每段还要跑双 voice(Claude subagent + Codex)，不能因为 plan 看起来 "差不多了" 就跳过任何一段或把某个 section 压缩成一行总结。完整审查本身就是 autoplan 的核心价值。
- **User Sovereignty**：Premise 确认与 User Challenge(两模型一致建议改变用户原定方向)必须走 AskUserQuestion 交给用户最终拍板。autoplan 的 6 个 principle 只用于自动化 taste 级的中间决策，**不可** 用来替用户做战略/范围决策。
...
Read the `/office-hours` skill file at `<skill_dir>/actions/office-hours.md` using the Read tool.
...
- `<skill_dir>/actions/plan-ceo-review.md`
- `<skill_dir>/actions/plan-design-review.md` (only if UI scope detected)
- `<skill_dir>/actions/plan-eng-review.md`
...
Follow `<skill_dir>/actions/plan-ceo-review.md` — all sections, full depth.
...
Follow `<skill_dir>/actions/plan-design-review.md` — all 7 dimensions, full depth.
Override: every AskUserQuestion → auto-decide using the 6 principles.
...
Follow `<skill_dir>/actions/plan-eng-review.md` — all sections, full depth.
Override: every AskUserQuestion → auto-decide using the 6 principles.
...
本 action 的关键输出必须写入 `{SPECS_DIR}/autoplan.md`：
...
```
✓ autoplan 完成。报告已写入 {SPECS_DIR}/autoplan.md
```
