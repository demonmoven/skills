...
> 移植自 [gstack/learn](https://github.com/garrytan/gstack/blob/main/learn/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/learn.md`。
>
> ⚠️ **本仓库特殊说明**：本 skill 的"跨会话学习记忆"依赖 `~/.gstack/projects/{slug}/learnings.jsonl` 以及 `~/.claude/skills/gstack/bin/gstack-learnings-search` / `gstack-learnings-log` 等上游二进制。在本仓库版本中，保留原始路径，即依赖用户独立安装上游 gstack 才能完整使用 show/search/prune/export/stats/add 子命令。若没有上游 gstack，仍可以把本次整理或用户手动添加的学习项以文本形式落到 `{SPECS_DIR}/learn.md` 作为快照。

## 参数

- `ARG`(可选)：子命令及其参数。支持 `search <query>`、`prune`、`export`、`stats`、`add`；空参数等价于 `show recent`(默认)。
...
- **Search Before Building**：在给出任何新建议前，必须先 `learnings-search` 历史学习，看看这个项目已经踩过哪些坑、沉淀过哪些约定。先看历史再建议，而不是从零复述"最佳实践"。
- **User Sovereignty**：prune/删除学习、导出到 CLAUDE.md 这类会长期影响团队记忆的动作都要走 AskUserQuestion 让用户拍板，不要自动执行。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/learn.md`：

- 若文件已存在则**在文件末尾追加**新一轮的输出(用 `## 第 N 次执行 ({YYYY-MM-DD HH:MM:SS+TZ})` 作为分隔标题)
- 若文件不存在则创建并写入完整结构
- 本次执行涉及的子命令、查询、prune 决策、新增条目都要落盘；若用户选择导出到 CLAUDE.md，原 CLAUDE.md 的改动也在 learn.md 中用 diff/摘要记录下来

执行完成后，输出一行简短摘要给用户：

```
✓ learn 完成。报告已写入 {SPECS_DIR}/learn.md
```
