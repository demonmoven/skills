...
> 移植自 [gstack/land-and-deploy](https://github.com/garrytan/gstack/blob/main/land-and-deploy/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/land-and-deploy.md`。
...
- **User Sovereignty**：生产环境部署是高风险操作，merge / deploy / verify 每一步都必须征询用户。
- **Investigation Before Fix**：部署失败时先调查根因（CI 日志、健康检查、监控指标），再决定回滚还是修复。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/land-and-deploy.md`：
...
```
✓ land-and-deploy 完成。报告已写入 {SPECS_DIR}/land-and-deploy.md
```
