...
> 移植自 [gstack/ship](https://github.com/garrytan/gstack/blob/main/ship/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/ship.md`。
...
- **Boil the Lake**：test + review + VERSION + CHANGELOG + push + PR 全部完整跑完，不在任何一步偷工减料。
- **User Sovereignty**：push 与 PR 创建前必须征询用户；遇到敏感文件改动（.env、credentials）必须暂停反问。
- **Atomic Commits**：每一处改动独立 commit，便于后续 review。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/ship.md`：
...
```
✓ ship 完成。报告已写入 {SPECS_DIR}/ship.md
```
