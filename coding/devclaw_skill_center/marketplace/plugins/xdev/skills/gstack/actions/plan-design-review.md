...
> 移植自 [gstack/plan-design-review](https://github.com/garrytan/gstack/blob/main/plan-design-review/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/plan-design-review.md`。

## 参数

- `ARG`(可选)：对 plan / 设计稿进行 UI/UX/视觉评审：信息层级、交互状态、边界情况、subtraction、品牌一致性等多维评分。可被指向某个 plan 文件路径，或在当前 feature 分支自动定位 SPECS_DIR 下的 plan。

## 入参校验

读取并执行 `<skill_dir>/prompts/resolve_action_target.md`，传入：
- `ACTION_NAME = "plan-design-review"`
- `ARG = {用户传入的参数}`
...
- **Boil the Lake**：评审要把所有维度（信息层级、交互态、边界情况、品牌、可访问性）都打到 10 分，缺一个维度视为未完成。
- **User Sovereignty**：设计选择（视觉风格、版式、配色）的最终决定权在用户，本 action 只负责提出问题与给出多组备选方向。
- **Search Before Building**：评审前先读 DESIGN.md、现有 component library、现有页面截图，避免提出与现有体系冲突的方案。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/plan-design-review.md`：
...
```
✓ plan-design-review 完成。报告已写入 {SPECS_DIR}/plan-design-review.md
```
