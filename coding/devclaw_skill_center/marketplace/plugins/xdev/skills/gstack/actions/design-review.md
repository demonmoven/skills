...
> 移植自 [gstack/design-review](https://github.com/garrytan/gstack/blob/main/design-review/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/design-review.md`。
...
- `ARG`（可选）：详见上游 description 与下方工作流。
...
执行本 action 前，读取 `<skill_dir>/resources/ethos.md` 并将其内容作为隐含约束。关键原则：

- **Boil the Lake**：80 项视觉审计全部跑完，不挑选只跑"重点项"；AI Slop 检测、可访问性、动效都要审。
- **Atomic Commits**：每发现一个问题独立修复独立 commit，便于后续 review 与回滚。
- **User Sovereignty**：审查发现的"品味问题"必须征询用户而非自动修复。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/design-review.md`：

- 若文件已存在则**在文件末尾追加**新一轮的输出（用 `## 第 N 次执行 ({YYYY-MM-DD HH:MM:SS+TZ})` 作为分隔标题）
- 若文件不存在则创建并写入完整结构
- 该 action 产生的截图、日志等附件统一放到 `{SPECS_DIR}/artifacts/`

执行完成后，输出一行简短摘要：

```
✓ design-review 完成。报告已写入 {SPECS_DIR}/design-review.md
```
