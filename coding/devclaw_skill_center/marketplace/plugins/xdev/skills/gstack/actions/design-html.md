...
> 移植自 [gstack/design-html](https://github.com/garrytan/gstack/blob/main/design-html/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/design-html.md`。
...
- **Boil the Lake**：生产级 HTML，不交付 mockup 或半成品；动效、响应式、可访问性都要包含。
- **User Sovereignty**：技术栈选择（React / Vue / Svelte / 纯 HTML）由用户拍板，design-html 只负责按需求实现。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/design-html.md`：
...
```
✓ design-html 完成。报告已写入 {SPECS_DIR}/design-html.md
```
