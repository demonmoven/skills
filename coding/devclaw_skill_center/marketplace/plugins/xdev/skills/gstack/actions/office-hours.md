...
> 移植自 [gstack/office-hours](https://github.com/garrytan/gstack/blob/main/office-hours/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/office-hours.md`。

## 参数

- `ARG`(可选)：用户想要 brainstorm 的产品想法、问题描述或方向。可为空——若为空则进入 Phase 1 Context Gathering 自动询问目标。
...
- **Boil the Lake**：office-hours 的对话不能止于第一个想法，必须穷尽 6 个 forcing question 与 alternatives，找到完整的设计文档而非半成品。
- **Search Before Building**：在提建议前先做 landscape research，看看世界已经怎么解这个问题，再判断是否需要 first-principles 重构。
- **User Sovereignty**：所有方向、premises、approach 选择都必须经过 AskUserQuestion 让用户拍板，office-hours partner 只负责挑战与建议，绝不代替用户决定。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/office-hours.md`：
...
```
✓ office-hours 完成。报告已写入 {SPECS_DIR}/office-hours.md
```
