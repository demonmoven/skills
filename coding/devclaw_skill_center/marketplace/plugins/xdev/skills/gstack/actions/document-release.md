...
> 移植自 [gstack/document-release](https://github.com/garrytan/gstack/blob/main/document-release/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/document-release.md`。

## 参数

- `ARG`(可选)：本次 release 需要重点关注的文档范围或改动描述。可为空——默认在当前 feature 分支上对全部 `.md` 文档做一次系统审计。

## 入参校验

读取并执行 `<skill_dir>/prompts/resolve_action_target.md`，传入：
- `ACTION_NAME = "document-release"`
- `ARG = {用户传入的参数}`
...
- **Boil the Lake**：document-release 不是只修 README，必须遍历仓库里所有 `.md`、CHANGELOG、VERSION、TODOS、CLAUDE.md、ARCHITECTURE.md、CONTRIBUTING.md 等文档，把本轮 diff 覆盖到的事实全部同步一遍。
- **Never Clobber CHANGELOG**：CHANGELOG 是 `/ship` 从真实 diff 写出的 source of truth，只能做 wording polish，绝不覆盖/重写/再生成；VERSION 绝不静默 bump，必须走 AskUserQuestion。
...
|--------|---------|-----|------|--------|----------|
| CEO Review | `<skill_dir>/actions/plan-ceo-review.md` | Scope & strategy | 0 | — | — |
| Codex Review | `/codex review` | Independent 2nd opinion | 0 | — | — |
| Eng Review | `<skill_dir>/actions/plan-eng-review.md` | Architecture & tests (required) | 0 | — | — |
| Design Review | `<skill_dir>/actions/plan-design-review.md` | UI/UX gaps | 0 | — | — |
...
本 action 的关键输出必须写入 `{SPECS_DIR}/document-release.md`：
...
```
✓ document-release 完成。报告已写入 {SPECS_DIR}/document-release.md
```
