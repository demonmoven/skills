...
> 移植自 [gstack/connect-chrome](https://github.com/garrytan/gstack/blob/main/connect-chrome/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/connect-chrome.md`。
>
> ⚠️ **前置依赖**：需要 gstack 上游的 `browse` 二进制 + Chrome 浏览器扩展（参考上游 README）。在 devclaw 环境中若 `~/.claude/skills/gstack/browse/dist/browse` 或 `~/.claude/skills/gstack/extension/manifest.json` 不存在，需要先从 garrytan/gstack 仓库 clone 并运行 `./setup` 构建。

## 参数

- `ARG`（可选）：用户希望打开/连接的目标 URL，或希望在 headed Chrome 里执行的具体任务描述。可为空——若为空则只做连接 + Side Panel 引导，不自动导航到任何页面。

## 入参校验

读取并执行 `<skill_dir>/prompts/resolve_action_target.md`，传入：
- `ACTION_NAME = "connect-chrome"`
- `ARG = {用户传入的参数}`
...
- **User Sovereignty**：connect-chrome 会启动一个可见的 headed Chrome 并接管浏览器控制，所有 click / goto / fill 都在用户眼前发生。任何会改变用户数据或登录态的操作都必须经过 AskUserQuestion 让用户明确同意，绝不悄悄代替用户做决定。
- **Concreteness is the standard**：每一步要点名具体的端口（34567）、具体的窗口（Playwright Chromium 而不是用户的常规 Chrome）、具体的扩展路径，让用户能一眼确认控制的是哪个浏览器。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/connect-chrome.md`：

- 若文件已存在则**在文件末尾追加**新一轮的输出（用 `## 第 N 次执行 ({YYYY-MM-DD HH:MM:SS+TZ})` 作为分隔标题）
- 若文件不存在则创建并写入完整结构
- 该 action 产生的截图、Side Panel 状态记录等附件统一放到 `{SPECS_DIR}/artifacts/`，并在产物文件中以相对路径引用

执行完成后，输出一行简短摘要：

```
✓ connect-chrome 完成。报告已写入 {SPECS_DIR}/connect-chrome.md
```
