...
> 移植自 [gstack/benchmark](https://github.com/garrytan/gstack/blob/main/benchmark/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/benchmark.md`。

## 参数

- `ARG`(可选)：性能回归检测的目标 URL 与模式。支持 `<url>`、`<url> --baseline`、`<url> --quick`、`<url> --pages /,/dashboard`、`--diff`、`--trend` 等形式。
...
- **Boil the Lake**：benchmark 不止跑一次冒烟，必须覆盖多页面、多指标(TTFB/FCP/LCP/bundle/requests)、多模式(baseline/compare/trend)，把性能画像画完整。
- **Search Before Building**：在制定新阈值或优化建议前，先读取 `.gstack/benchmark-reports/baselines/` 下已有基线，基于历史数据判断当前是回归还是噪声。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/benchmark.md`：
...
```
✓ benchmark 完成。报告已写入 {SPECS_DIR}/benchmark.md
```
