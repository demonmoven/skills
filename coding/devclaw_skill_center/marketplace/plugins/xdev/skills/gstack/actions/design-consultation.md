...
> 移植自 [gstack/design-consultation](https://github.com/garrytan/gstack/blob/main/design-consultation/SKILL.md) v0.15.1.0
> 上游 frontmatter / preamble / telemetry / proactive 配置已删除，原工作流逻辑保留。
> 中间产物统一落到 `{PRIMARY_REPO}/docs/xdev/gstack/{FEATURE_NAME}/design-consultation.md`。
...
- `ACTION_NAME = "design-consultation"`
- `ARG = {用户传入的参数}`
...
- **Search Before Building**：先研究竞品景观（同类产品的字体、配色、间距、动效），再提"安全选择 + 创意风险"两套方案。
- **User Sovereignty**：所有设计选择（品牌方向、视觉风格）必须经用户拍板，design-consultation 只负责挑战与建议。
...
本 action 的关键输出必须写入 `{SPECS_DIR}/design-consultation.md`：
...
```
✓ design-consultation 完成。报告已写入 {SPECS_DIR}/design-consultation.md
```
