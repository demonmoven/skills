...
**失败时不要跳到"网络故障/账号池挂了/JWT 失效/服务端 bug"等猜想。先拿到真实证据。**

### Step 0a：先验证 E2E fixture 配置

诊断业务断言失败 / occupy_account 卡死 / 401 / 400 之前，**先打印相关 fixture 配置** ——  看看字段是否放错位置（反模式见 `qa-model-effect-json-schema.md`）：

1. `pool_user_name` / `tag_name_list` / `bot_id` 必须在 `params` 内，不在顶层
2. `params.ENV_LABEL` 必须等于当前 `TCE_LANE`（例 `ppe_wutingjia_test`），**不能是空字符串**；空字符串 ⇒ `x-tt-env` header 缺失 ⇒ 请求落到 prod ⇒ 测的是线上老逻辑而非本轮部署的 PPE 代码

验证方法：
```bash
python3 -c "import json; d=json.load(open('<fixture_path>')); print(json.dumps(d[0], ensure_ascii=False, indent=2))" | head -30
```

检查点：`params` dict 里有 `pool_user_name` / `tag_name_list` / `ENV_LABEL`？`ENV_LABEL != ""`？

### Step 0b：确认是否触达目标泳道

看 pytest.log 中响应 header `X-Backend:` 或 SSE 响应的 `x-tt-env`；应当含目标泳道名（如 `ppe_wutingjia_test`）。若是 `prod|*` ⇒ 染色未生效 ⇒ 回到 Step 0a 修 `params.ENV_LABEL`。

### Step 0c：占用账号失败？先看 HTTP 响应 body

如果大量 case 挂在 occupy_account 阶段，**不要**立即归因到"账号池挂了/JWT 过期/网络不通"。先看 `result.status_code, result.text =` 打印的真实响应 body：
- `400 "Params Err, msg=user name can not by empty"` ⇒ 测试代码给 body 传了 None/空字符串，通常是 fixture 把 `pool_user_name` 放错位置（Step 0a）
- `401 / 403` ⇒ 真的 JWT 失效
- 超时 ⇒ 再考虑网络问题
- `200 code:0` ⇒ 占用成功，看后续业务断言

**若 pytest.log 里看不到真实响应 body**：pytest 默认捕获 stdout，print 不进 log。重跑时加 `-s` flag。

进入步骤 1 之前，上述 3 项必须先排除。
...
### 步骤 2.5：通过 LogID 查询服务日志（可选）

当 `failed_cases.jsonl` 中的失败用例包含非空的 `log_ids` 字段时，查询服务端日志获取额外的调试上下文。
...
参考 `$BYTEDCLI_SKILLS_DIR/bytedance-log/SKILL.md` 获取日志查询命令的用法。执行前先通过 `--help` 确认当前版本参数。

关键点：
- BOE 环境必须指定 BOE 站点
- 传入被测服务的 PSM，用于只查看被测服务的日志
- 将日志输出到文件，避免 stdout 过大导致上下文过载

**轮询逻辑**（日志可能延迟写入）：

日志写入可能比测试执行有延迟。对每个 LogID 实施重试（最多 10 次，渐进退避 10s→30s，累计最大 ~4 分钟）。

**日志过滤策略**（避免日志过多导致上下文过载）：

1. `--psm` 参数已在查询时过滤，只返回被测服务的日志
2. 对输出文件做**本地关键词过滤**，提取有诊断价值的行：

```bash
# 过滤出错误相关的关键行（含前后 3 行上下文）
...
# 如果过滤后为空（没有匹配到错误关键词），保留原始日志的前 200 行作为兜底
if [ ! -s /tmp/logid_filtered_<LOGID>.log ]; then
  head -200 /tmp/logid_raw_<LOGID>.log > /tmp/logid_filtered_<LOGID>.log
fi
```

3. 使用过滤后的日志内容进行分析，原始日志保留在临时文件中备查

**处理规则**：
- 每个失败用例最多查询 **3 个**不同的 LogID（避免过多 API 调用）
- 如果多个失败用例共享同一个 LogID，只查询一次
- 没有 `log_ids` 的用例跳过日志查询
- 超时后继续分析，不阻塞整体流程——日志只是辅助证据
- 将过滤后的日志内容按 LogID 索引存储，供后续步骤 4 分析时引用
...
## Workspace 模式下的多 PSM 日志查询

Stage 3 的 LLM 判断根因时需要读线上日志。workspace 模式下嗅探到的 target 可能有多个 PSM。流程：

1. 读 `$OUTPUT_DIR/detected_targets.jsonl`，提取所有 PSM
2. 对每个 failed_case 的 LogID（从 `allure_detail.attachments` 里找）：
   - **必查**：失败 case 的"direct upstream" PSM（从 test 文件 import path 推测）
   - **补查**：其它 target PSM（可能是链路下游）
3. 日志查询命令（参考 `$BYTEDCLI_SKILLS_DIR/bytedance-log/SKILL.md`）：
   ```
   # 建议先 --help 确认参数
   bytedcli log search --psm $PSM --logid $LOGID --env boe --env-label $TCE_LANE --tail 500
   ```
4. 合并多路日志按时间排序，供 LLM 统一分析

## 根因分类（补充 pytest 场景）

在原 5 分类基础上，明确 pytest 常见具体模式：

| 根因 | pytest 场景特征 |
|---|---|
| `[BIZ]` | 业务响应内容错（拒答、字段值错、流式返回格式错） |
| `[TEST]` | 断言过严（`expected.reply == "xxx"` 但模型返回 "xxx。"）；JSON 账号池名过时；测试数据引用了已下线的 bot_id |
| `[IFACE]` | IDL 变了但 test_data JSON 里缺了新字段；或测试用旧 version_id |
| `[INFRA]` | PPE 泳道未正确部署（`ENV_LABEL` header 未生效）；账号池耗尽；trace 系统超时 |
| `[FLAKY]` | LLM 输出不稳定（偶尔触发拒答）、网络抖动 |
