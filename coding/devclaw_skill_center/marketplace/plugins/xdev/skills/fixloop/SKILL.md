---
name: fixloop
description: 全链路测试修复循环：测试生成 → 部署 → 测试 → 分析 → 修复。支持参数控制各环节开关和迭代次数。
...
> **\* 上下文感知**：标记为"是\*"的参数在用户未显式传入时，应**优先从当前对话上下文中推断**，而非立即询问用户。Fixloop 经常作为 speckit 等上游 skill 的后续步骤运行，此时上下文中通常已经包含了相关信息。
...
> **使用 Agent 工具启动 subagent 执行。** Prompt 中指定读取 `references/craft-stage2-e2e-generation.md` 作为执行指南。此步骤涉及大量代码探索和生成，必须使用 subagent。
...
## Stage 1.5: AGW IDL 同步 [主 context 执行]
...
> **使用 Agent 工具启动 subagent 执行。** Prompt 中指定读取 `references/test-execution.md` 作为执行指南，并传入下方所有参数。Subagent 完成后需返回测试通过/失败统计。
...
- E2E 测试生成流程：`references/craft-stage2-e2e-generation.md`
- 单元测试生成流程：`references/craft-stage3-unit-tests.md`
- 覆盖度差距分析：`references/craft-coverage-analysis.md`
- AGW IDL 同步流程：`references/agw-idl-update.md`
- 部署流程：`references/deploy-procedure.md`
- 测试执行流程：`references/test-execution.md`
- go test -json 解析规则：`references/go-test-json-format.md`
- 失败根因分析流程：`references/failure-analysis.md`
- 代码修复流程：`references/fix-procedure.md`
- TCE 命令参考：`references/tce-commands.md`
- TCE 常见问题排查：`references/tce-troubleshooting.md`
