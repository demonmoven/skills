...
fix-loop 在 `$OUTPUT_DIR` 下组织所有产出数据。Stage 0（craft）的产出放在 `craft/` 子目录，每轮迭代在独立的 `iteration_N/` 子目录中，确保数据隔离不被覆盖。
...
纯文本文件，每行一个测试条目，格式为 `TestFuncName ./relative/dir`。由 fix-loop Stage 0 从 `generated_test_cases.jsonl` 自动提取生成。
...
| PSM | fix-loop 参数 | Stage 3 (failure-analysis `--psm` 过滤) | Agent prompt 参数 |
| BYTEDCLI_SITE | fix-loop 参数 | Stage 3 (failure-analysis `--site` 多站点) | Agent prompt 参数 |
