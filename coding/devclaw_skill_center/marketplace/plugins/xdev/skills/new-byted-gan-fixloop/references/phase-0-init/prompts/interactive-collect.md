...
[预热] cwd 扫描（收集所有候选）
   ↓
[Step 1/4] 仓库与分支 (BUSINESS_REPO / BRANCH / TEST_REPO)
   ↓
[Step 2/4] 输入源 (SPEC_DIR / IDL_REPO / IDL_BRANCH)
   ↓
[Step 3/4] 字节内场参数 (PSM)
   ↓
[Step 4/4] 高级选项 + 最终汇总确认
   ↓
...
4. **已显式给出的参数自动跳过对应 question**。比如用户调用 `/new-byted-gan-fixloop PSM=stone.cozeloop.prompt`，PSM 的 question 不发出。
...
```bash
# 例：用户调用 /new-byted-gan-fixloop PSM=stone.cozeloop.prompt BRANCH=main
EXPLICIT_PARAMS=()
