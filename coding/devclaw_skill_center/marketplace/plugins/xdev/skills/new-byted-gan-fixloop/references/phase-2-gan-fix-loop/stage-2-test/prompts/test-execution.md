...
`go test -json` 输出为每行一个 JSON 对象。详细解析规则见 `../../../_shared/go-test-json-format.md`。
...
**LogID 提取**：解析 `Action == "output"` 事件时，从 `Output` 内容中用正则提取 LogID（来自 HTTP 响应头 `X-Tt-Logid`），写入 `log_ids` 字段。详见 `../../../_shared/go-test-json-format.md`。
