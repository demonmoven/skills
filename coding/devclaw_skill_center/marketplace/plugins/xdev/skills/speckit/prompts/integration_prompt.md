...
# integration 指令（系统集成设计）

> 根据 Spec 文档及前序分析产物，生成系统内部对外的集成方案文档
...
1. Integration 文档：INTEGRATION_DOC，文档名必须为 "integration"
...
### 步骤 2：生成 Integration 文档

1. 向用户报告 MESSAGE 提示 "【当前状态】开始 >> 生成 Integration 文档..."

结合当前上下文生成 INTEGRATION_DOC，内容必须涉及**此次需求相关**的系统内部对外的集成方案，包括但不限于：

* RPC 外调（调用外部 RPC 服务）
* Docker Compose 里新加的基础服务（以供系统可以接入并调用）
* 系统接入外部的 HTTP 服务
* 其他外部依赖服务的集成
* **重要**：**明确标出是现存(不改)/增/删/改中的哪一个**
* **注意**：**不包括**系统对外暴露的服务（API 等，这是 CONTRACTS_DOC 的内容）
...
将文档输出到 INTEGRATION_DOC。

2. 向用户报告 MESSAGE 提示 "【当前状态】完成 >> 生成 Integration 文档"
