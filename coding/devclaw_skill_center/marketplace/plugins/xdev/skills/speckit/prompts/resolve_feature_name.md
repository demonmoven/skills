...
> 通用的 FEATURE_NAME 解析逻辑，被所有 action 在入参校验阶段引用。

## 输入参数

1. `ACTION_TYPE`：当前 action 类型
   - `"specify"` 或 `"run"`：支持新建 feature（带参数=新需求描述，不带参数=续接）
...
2. `SPECS_DIR`：`{PRIMARY_REPO}/docs/xdev/speckit/{FEATURE_NAME}` 的绝对路径
3. `IS_NEW_FEATURE`：布尔值，是否为新建 feature（仅 specify/run 模式可能为 true）
...
#### ACTION_TYPE = "specify" 或 "run"

参数视为**新需求描述**，进入步骤 3（无意义检测）。

#### ACTION_TYPE = "other"

参数视为 **FEATURE_NAME**，校验 `docs/xdev/speckit/{ARG}/` 目录是否存在：
- 存在 → `FEATURE_NAME = {ARG}`，`IS_NEW_FEATURE = false`，**解析完成**
- 不存在 → 报错提示用户："`docs/xdev/speckit/{ARG}/` 目录不存在，请确认 FEATURE_NAME 是否正确"

---

### 步骤 3：无意义检测（仅 specify/run）
...
### 步骤 4：新建 Feature 流程（仅 specify/run）
...
2. 创建 specs 目录（在主仓库根目录下）：
   ```bash
   mkdir -p {PRIMARY_REPO}/docs/xdev/speckit/{FEATURE_NAME}
   ```
3. 将 `{USER_INPUT}`（原始参数原文）写入 `{PRIMARY_REPO}/docs/xdev/speckit/{FEATURE_NAME}/feature.md`

设置返回值：
- `SPECS_DIR = {PRIMARY_REPO}/docs/xdev/speckit/{FEATURE_NAME}` 的绝对路径
- `IS_NEW_FEATURE = true`
...
- **优先**检查 `{PRIMARY_REPO}/docs/xdev/speckit/{branch}/` 目录是否存在
  - 存在 → `FEATURE_NAME = {branch}`，`SPECS_DIR = {PRIMARY_REPO}/docs/xdev/speckit/{branch}`，`IS_NEW_FEATURE = false`，**解析完成**
- **兼容期 fallback**：若新目录不存在但旧的 `{PRIMARY_REPO}/speckit/{branch}/` 存在 → 提示用户运行 `mv speckit docs/xdev/speckit` 迁移后重试
- 都不存在 → 进入 5c

#### 5c. 扫描 docs/xdev/speckit/ 目录（兼容期内同时扫描旧的 speckit/）

```bash
ls -d "$PRIMARY_REPO"/docs/xdev/speckit/*/ 2>/dev/null
ls -d "$PRIMARY_REPO"/speckit/*/ 2>/dev/null  # 兼容期 fallback：扫描旧目录
```
...
- **有子目录** → 列出所有 feature 目录名，使用交互式选择让用户选择：
  ...
  {仅 specify/run 模式展示} N. [输入新需求描述]

  请输入序号：
  ```

  - 用户选择已有 feature → `FEATURE_NAME = 选中值`，`SPECS_DIR = {PRIMARY_REPO}/docs/xdev/speckit/{FEATURE_NAME}`，`IS_NEW_FEATURE = false`，**解析完成**
  - 用户选择"输入新需求描述"（仅 specify/run）→ 提示用户输入需求描述 → 将输入作为 `ARG` 回到步骤 3

- **无子目录**（`docs/xdev/speckit/` 与旧的 `speckit/` 都为空或不存在）：
  - ACTION_TYPE = "specify" 或 "run" → 提示用户："当前没有已创建的 feature，请输入需求描述或文档链接" → 将输入作为 `ARG` 回到步骤 3
  - ACTION_TYPE = "other" → 报错："当前没有已创建的 feature，请先运行 `/speckit run <需求描述>` 或 `/speckit specify <需求描述>` 创建 feature"
