...
1. **Discover modules**: Scan the project root directory to identify top-level modules. For Go projects, look for:
   - Directories under `cmd/` (service entry points)
   - Directories under `internal/` (internal packages)
   - Directories under `pkg/` (public packages)
   - Standalone Go modules (directories with their own `go.mod`)
...
3. **Do not skip modules**: Every module directory that contains `.go` source files must have a corresponding documentation file. If a module has no source files, skip it.
...
| File | Responsibility | Required |
|------|---------------|----------|
| `background.md` | Business background introduction (CSP customer service platform, international e-commerce, etc.) | No |
| `processes.md` | Core business process descriptions (ticket routing, knowledge base management, etc.) | No |

### modules/ — Per-Module Documentation

| File | Responsibility | Required |
|------|---------------|----------|
| `<module-name>.md` | Per-module detailed documentation: architecture, breakdown, core flows, code index | Yes |

### Template Reference Table

Each docs file has a corresponding reference template that defines its content structure:
