...
func runCase(t *testing.T, params CleanParams, posts ...func(*testing.T, CleanParams)) {
	fmt.Println("start run case")
	if params.RepoDir == "" {
...
type CleanParams struct {
	...
	NoCommit     string `flag:"no-commit"`
	RepoAppMains string `flag:"repo-app-mains"`
...
func (p CleanParams) buildFlags() []string {
	...
	flags = append(flags, cleanScript())
...
func cleanScript() string {
	return filepath.Join(workDir, "worker", "script", "clean_script.sh")
}

func checkExpectedFiles(t *testing.T, params CleanParams) {
	root := params.RepoDir
...
func runCleanScript(params CleanParams) {
	defer func() {
...
func prepareRunEnv() {
	...
	codeCleanTestDir = getCodeCleanTestDir()
	...
	util.SetCleanerIntegrationTestEnv()
	newWorkEnv = true
...
func printRunEnv() {
	...
	fmt.Println("- EndpointTestDir:", codeCleanTestDir)
...
func getCodeCleanTestDir() string {
...
func mustBuildWorker() {
	...
	must(os.Chdir(filepath.Join(codeCleanTestDir, "../../app/cleaner/worker/v2.1")))
...
func mustBuildCleaner() {
	...
	must(os.Chdir(filepath.Join(codeCleanTestDir, "../../cmd/cleaner")))
