...
func runGoCleanTask(ctx context.Context, reportInterval time.Duration, task *model.CodeCleanTask) {
	var (
		err              error
		stdErr, stdOut   []byte
	...
	defer func() {
		...
		if strings.Count(goVersion, ".") == 2 {
			task.TaskOpt = strPtr("go" + goVersion[:strings.LastIndex(goVersion, ".")])
		} else {
			task.TaskOpt = strPtr("go" + goVersion)
		}
		...
		if err != nil {
			...
			if errors.As(err, &existErr) {
				if status, ok := existErr.Sys().(syscall.WaitStatus); ok && status.ExitStatus() == ShellExitGoVersionError {
					// 退出码111代表当前worker的版本与改仓库的版本不匹配，保持-1状态，同时更新task_opt为对应的go版本
					task.TaskStatus = int64Ptr(-1)
					return
				}
			}
	...
	go func() {
		if reportInterval <= time.Second || isTest(ctx) {
	...
	if repoName == "" {
		err = fmt.Errorf("go worker: repo name is empty")
		return
	}
	...
	commandString := fmt.Sprintf("[%s]full command: %s,args: %v", time.Now().Format("2006-01-02T15:04:05"),
		cmd.Path, cmd.Args)
	if task.Extra != nil {
		commandString += fmt.Sprintf(", skip_create_mr:%v", task.Extra.SkipCreateMR)
	}
	...
	go func() {
		...
		for scannerOut.Scan() {
			...
			} else if strings.HasPrefix(out, "_base_commit_id:") {
				baseCommitID = strings.TrimSpace(out[16:])
			}
