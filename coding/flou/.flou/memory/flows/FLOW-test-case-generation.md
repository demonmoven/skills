...
1. navigate `<URL>`
2. wait 2000
3. screenshot -o "$RESULT_DIR/screenshots/初始页面.png"
4. hover text="用户输入"
5. wait 1500
6. screenshot -o "$RESULT_DIR/screenshots/用户输入弹窗.png"
```
...
```bash
go run ./cmd/flou test record-req "<URL>" \
  --task-id "$TASK_ID" \
...
# 截图（支持中文命名）
agent-browser --cdp 9222 screenshot -o "$RESULT_DIR/screenshots/<中文名称>.png"
```
...
| 截图时机 | 截图名称示例 | 说明 |
|----------|--------------|------|
| 页面加载完成 | `初始页面.png` | 记录测试开始时的页面状态 |
| 操作前 | `点击提交前.png` | 记录关键操作前的状态 |
| 弹窗出现 | `确认弹窗.png` | 记录弹窗内容 |
| 操作后 | `提交成功提示.png` | 记录操作后的结果 |
| 关键结果 | `数据列表.png` | 记录关键数据展示 |
| 异常情况 | `错误提示.png` | 记录异常或错误信息 |

**截图命名规范：**
- 使用描述性的中文名称，清晰表达截图内容
- 格式：`<操作/状态描述>.png`
- 示例：`用户输入弹窗.png`、`Query窗口.png`、`查询结果列表.png`
...
```bash
go run ./cmd/flou test stop-record -s <SESSION_ID>
```
...
| 截图名称 | 截图路径 | 说明 |
|----------|----------|------|
| 初始页面 | $RESULT_DIR/screenshots/初始页面.png | 测试开始时的页面状态 |
| 用户输入弹窗 | $RESULT_DIR/screenshots/用户输入弹窗.png | 用户输入弹窗内容 |
| Query窗口 | $RESULT_DIR/screenshots/Query窗口.png | Query窗口展示 |
| 提交成功提示 | $RESULT_DIR/screenshots/提交成功提示.png | 提交操作后的成功提示 |
| 查询结果列表 | $RESULT_DIR/screenshots/查询结果列表.png | 查询结果数据展示 |
...
# 检查录制会话
go run ./cmd/flou test list-sessions
```
