---
name: harness-obs-init
description: "为任意 Go 仓库建立本地观测栈。基于字节 logs/v2 SDK，接入结构化日志推送、OTel 链路追踪和极简基础设施（VictoriaLogs + VictoriaTraces）。产出 observability 包代码、docker-compose 编排、日志字段标准、排障查询模板。当用户提到观测栈、obs init、本地可观测、接入 VictoriaLogs、接入链路追踪、harness obs、日志规范时激活。"
license: Apache-2.0
metadata:
  author: guoshuai.030
  version: "1.4"
