---
name: transaction_coding_workflow
description: 交易服务开发workflow - 当用户进行一个完整的需求开发时执行
---
...
调用本 skill 后，按以下流程自动执行。**不需要子命令**——直接传入需求描述或显式传 `--task-id <task_id>`（续接已有开发任务）。


### Step 0：task_id准备

#### 显式传入了`--task-id <task_id>`（优先级最高）
1. `git rev-parse --show-toplevel` → `PRIMARY_REPO`，失败则报错终止
2. 检查 `{PRIMARY_REPO}/docs/coding_task/active/<task_id>/` 目录是否存在。
   - 不存在 → 提示用户task_id不存在后，报错终止
   - 存在 → 则task_id=<task_id>

#### 显式传入了 `$ARGUMENTS`
1. `git rev-parse --show-toplevel` → `PRIMARY_REPO`，失败则报错终止
2. **无意义检测**：判断 `$ARGUMENTS` 是否能理解为有意义的软件需求描述或文档链接。随机字符、乱码、过于笼统的描述（如"做个东西"）视为无意义，报错反问用户。URL 格式一律视为有意义。
3. **生成 `feature_name`**：
   - 如果当前分支 **不是** `master` 或 `main`：默认使用当前分支名称作为`feature_name`。
   - 如果当前分支 **是** `master` 或 `main`：根据需求生成一个新的分支名称`feature_name`（生成逻辑：从用户输入中提取需求简要描述，翻译整理为 `{req_slug}`（最多 4 个单词，kebab-case，全小写）；获取当前时间戳 `{YYYYMMDD}`；分支名称=`{YYYYMMDD}-{req_slug}`；示例：`20200101-add_item_bind`），并在确认单中要求用户确认。
4. **分析需求**：从`{PRIMARY_REPO}/agents.md`中获取服务信息（PSM，IDL路径） ，从用户输入中提取关键信息（是否变更IDL等）。
5. **交互确认**：**必须**向用户展示以下确认表单（Markdown 格式），并等待用户确认 "yes" 或提供修改意见。
...
| **PSM** | `ad.star.xxx` | 目标服务 PSM |
| **分支名称feature_name** | `{YYYYMMDD}-{req_slug}` | 将使用的 Git 分支（如本地为 master，此处为建议的新分支名） |
...
4. **创建工作区 + 推分支**：
   - 创建分支，如果当前分支不是feature_name，执行`git checkout -b feature_name` 并 `push -u origin feature_name`，如果创建失败则直接终止
   - 创建工作区
   ⚠️ **MUST**：本步骤必须读取并按 `<skill_dir>/reference/create_task.md` 的指令**完整执行**
   读取并执行 `<skill_dir>/reference/create_task.md`，传入：
   ```text
...
查找 `.gdpa/` 目录下是否有已存在的会话（`{task-id}/status.json`）：

- **存在 `status.json`**：读取文件，根据 `current_phase` 和 phase `status` 恢复到对应阶段继续执行
- **不存在**：使用当前 `task_id` 初始化新的 `.gdpa/{task-id}/status.json`
...
> **task_id 传递**：若本次任务需要在多次执行之间串联 workflow 状态、日志或上下文，请复用同一个 `task_id`。如果当前 skill / Agent 已经提供了 `task_id`，**请直接复用，不要新建**。
>
> - **已有时优先复用**：不要重复创建 `task_id`。
> - **没有时再创建**：执行 `gdpa-cli create-session`。
> - **后续调用**：可以显式传 `--task-id <task_id>`，例如 `gdpa-cli run <agent> --task-id <task_id> --input '{...}'`。

# 基础研发流程手册

> **核心原则**：整个研发流程由 `.doc/coding_task/{task-id}/status.json` 驱动。每次执行前先读取状态文件，确认当前阶段；每步操作后更新状态文件。
> **task-id 约束**：`gdpa-cli run` 全局不再强制要求 `--task-id`，但本 workflow 必须在整条链路里显式复用同一个 `--task-id <task_id>`。CLI 不会自动把 task-id 注入到 Skill 输入；如果漏传或中途更换，会导致 `.doc/coding_task/{task-id}/status.json`、阶段产物、恢复点以及部分基于 Session 的排障信息断链。
...
### 0. 确定本次 workflow 使用的 task_id
- **当前对话或用户输入已提供 task_id**：直接复用这个值
- **未提供**：立即执行 `gdpa-cli create-session`，使用返回的 task_id
- **拿到后**：后续所有 `gdpa-cli run` 命令都必须显式追加同一个 `--task-id <task_id>`
...
5. **Session 隔离**：每次研发流程使用独立 `task-id`，互不干扰
