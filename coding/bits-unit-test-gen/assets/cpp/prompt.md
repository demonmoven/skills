...
| Naming          | `TEST(SuiteName, TestName)` or `TEST_F` (English only)              |
...
TEST_F(OrderServiceTest, CreateOrder_normalCreation) {
    // 正常创建订单
    MockDatabase mockDB;
...
```cpp
TEST_F(UserServiceTest, GetUser_normalFetch) {
    // 正常获取用户信息
    MockUserDAO mockDAO;
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        AddTestParam{"TwoPositiveNumbers", 1, 2, 3},
        AddTestParam{"PositivePlusNegative", 5, -3, 2},
        AddTestParam{"TwoNegativeNumbers", -1, -2, -3},
        AddTestParam{"AddZero", 10, 0, 10},
        AddTestParam{"BothZero", 0, 0, 0},
        AddTestParam{"LargeNumberAddition", INT_MAX - 1, 1, INT_MAX}
    ),
...
TEST_F(UserServiceTest, GetUser_normalFetchUserInfo) {
    // 正常获取用户信息
...
TEST_F(UserServiceTest, GetUser_notFoundReturnsNullptr) {
    // 用户不存在返回nullptr
...
TEST_F(UserServiceTest, GetUser_zeroIdReturnsNullptr) {
    // ID为零返回nullptr
    auto* result = service_->GetUser(0);
    EXPECT_EQ(result, nullptr);
}

TEST_F(UserServiceTest, GetUser_negativeIdReturnsNullptr) {
    // ID为负数返回nullptr
